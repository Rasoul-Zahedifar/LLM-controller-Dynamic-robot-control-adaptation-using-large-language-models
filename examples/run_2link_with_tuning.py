#!/usr/bin/env python3
"""
Example script for running 2-link manipulator simulation WITH LLM TUNING.

This script demonstrates the complete LLM-based controller tuning process,
where the LLM iteratively adjusts gains based on performance metrics.
"""
...
from llm_controller.visualization.animator import plot_results, create_animation
...
OUTPUT_DIR = './output_2link_tuning'  # Output directory for results

# Optional: Override initial gains (set to None to use defaults from SystemParams)
# Format: [k_p, k_d, k_i, landa_1, landa_2]
INITIAL_GAINS = None  # Use None for default, or e.g., [10.0, 5.0, 2.0, 0.1, 0.1] for custom
# ============================================================================


def main():
    """Run 2-link manipulator simulation with LLM tuning."""
    print("="*80)
    print("2-Link Manipulator Simulation with LLM-Based Controller Tuning")
    print("="*80)
    ...
    for attempt in range(MAX_TUNING_ATTEMPTS):
        ...
        try:
            create_animation(results[0], runner.l1, runner.l2, 
        ...
        satisfier_prompt = f"""
The 2-link manipulator controller has been tested with the following performance:
...
Is this performance satisfactory for a robotic manipulator tracking task? 
...
Please suggest new controller gains to improve performance. 
...
"""
    ...
    try:
        create_animation(final_results[0], final_runner.l1, final_runner.l2, 

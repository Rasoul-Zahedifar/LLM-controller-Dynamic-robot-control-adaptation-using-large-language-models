"""
2-Link Manipulator Controller Module

This module implements an adaptive sliding mode controller for a 2-link
...
class Controller:
    """
    Adaptive sliding mode controller for 2-link manipulator.
    ...
    def run(self, q, q_dot, X_d, X_d_dot, X_d_ddot, dt):
        ...
        Args:
            ...
            X_d: Desired end-effector position (2x1 array)
            X_d_dot: Desired end-effector velocity (2x1 array)
            X_d_ddot: Desired end-effector acceleration (2x1 array)
            dt: Time step (s)
        ...
        theta1, theta2 = q.flatten()
        theta1_dot, theta2_dot = q_dot.flatten()
        ...
        # Compute end-effector velocity (Jacobian * joint velocities)
        X_dot = np.array([
        ...
        M_x, C_x, G_x, F_x, B_x = self.dynamics.matrices(theta1, theta2, theta1_dot, theta2_dot)
        ...
        # Compute control force in task space (computed torque + sliding mode + adaptive terms)
        f_motor = (M_x @ v_dot + 
                   C_x @ v + 
                   G_x + 
                   F_x - 
                   self.k_d * s - 
                   self.d_hat * np.tanh(s) - 
                   self.kesi_hat * np.tanh(s))
